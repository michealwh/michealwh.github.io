
var notes = {
    "Morris Maulde": {
        key: "Morris Maulde",
        order: "chrono",
        chance: 5,
        startday: 5,
        notes: [
            {
                title: "#1 It is never enough.",
                description: 
                `I will always need more.\nThe job's rarely a bore.\nSo I keep working for this store.\nIt is never enough.`,
                image: false
            },
            {
                title: "#2 It is never enough.",
                description: 
                `Another dollar in my pocket.\nOne more coin to deposit.\nA greasy burger on the market.\nIt is never enough.`,
                image: false
            },
            {
                title: "#3 It is never enough.",
                description: 
                `Tirelessly I go on.\nWorking from dusk to dawn.\nAm I a moron?\nIt is never enough.`,
                image: false
            },
        ]
    },
    "Ranulf Gorrison": {
        key: "Ranulf Gorrison",
        order: "chrono",
        chance: 3,
        startday: 3,
        notes: [
            {
                title: "Pizza cutter",
                description: "Back and forth it goes.\nThe pizza is split up now.\nDinner is ready."
            },
            {
                title: "Come in.",
                description: "I do not impose.\nThere is fear of rejection.\nHe tells me come in."
            }
        ]
    },
    "Glorbdon": {
        key: "Glorbdon",
        order: "chrono",
        chance: 8,
        startday: 5,
        notes: [
            {
                title: "Untitled #1",
                description: "I hope this isn't my last burger.\nI'd be sad.",
                image: false
            },
            {
                title: "Untitled #2",
                description: "Do you know gratitude?",
                image: false
            }
        ]
    },
    "Grugory Hefert": {
        key: "Grugory Hefert",
        order: "chrono",
        chance: 2,
        startday: 4,
        notes: [
            {
                title: "Untitled #5",
                description: "I get tempted to get milk sometimes but I know it wouldn't be good for me."
            },
            {
                title: "Untitled #6",
                description: "Does the sadness really go away as we grow up or do we just get better at burying it?"
            },
            {
                title: "Untitled #7",
                description: "My children are to be my life, my heart, my soul, but they drain me."
            },
            {
                title: "Untitled #8",
                description: "I dream of freedom, but I'm not certain I'd use it if I had it.\nI didn't back then."
            }
        ]
    },
    "Marmaduke Gloo": {
        key: "Marmaduke Gloo",
        order: "chrono",
        chance: 3,
        startday: 6,
        notes: [
            {
                title: "Untitled #3",
                description: `Jellyfish aren't actually fish. They don't have fins or gills and instead swim by opening or closing their “bells.”`
            }
        ]
    },
    "Torbino Gloo": {
        key: "Torbino Gloo",
        order: "chrono",
        chance: 2,
        startday: 3,
        notes: [
            {
                title: "Untitled #4",
                description: "I will tell my brothers of your establishment."
            }
        ]
    },
    "Felippe Diaz": {
        key: "Felippe Diaz",
        order: "chrono",
        chance: 3,
        startday: 4,
        notes: [
            {
                title: "Untitled #42",
                description: "I had a dream last night. Two towers. One of light. Another of blight. On each stood a prince. One upholding perception. The other following ignorance. They hated each other."
            }
        ]
    }
}

export default notes